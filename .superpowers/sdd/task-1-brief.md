# Task 1: lib/download-core.js — 上下文无关的下载执行引擎

**Files:**
- Create: `lib/download-core.js`

**Interfaces:**
- Consumes: 无（纯逻辑，不依赖 chrome API）
- Produces:
  - `export const QMAP` — 画质码→文案映射
  - `export const DOWNLOAD_BASE` — `'bilibili_download'`
  - `export function sanitizeFilename(name) -> string`
  - `export async function getPlayUrl(aid, bvid, cid, qn=80, signal, fetchFn=fetch) -> dash data`
  - `export async function downloadStream(url, signal, notify, fetchFn=fetch) -> Blob`
  - `export async function mergeAudioVideo(ffmpeg, audioBlob, videoBlob, notify) -> Blob`
  - `export async function executeTask(taskId, videoInfo, qualityIdx, deps)`
    - deps: `{ getSettings, getFFmpeg, notify, saveBlob, signal, fetchFn }`
    - notify 签名 `(payload) => void`，payload: `{ phase, percent, label }`
    - saveBlob 签名 `(blob, filename, subdir) => Promise`
    - 成功 → 无返回值；getPlayUrl 失败(非中止) → 抛 `Error` 且 `err.code = 'NEEDS_PAGE'`；中止 → 抛 `DOMException('aborted','AbortError')`

- [ ] **Step 1: 创建 `lib/download-core.js`**
...
export async function getPlayUrl(aid, bvid, cid, qn = 80, signal, fetchFn = fetch) {
  ...
  const r = await fetchFn(url, { credentials: 'include', signal });
...
export async function downloadStream(url, signal, notify, fetchFn = fetch) {
  const r = await fetchFn(url, { credentials: 'include', signal });
  if (!r.ok) throw new Error('流 HTTP ' + r.status);
  ...
  while (true) {
    ...
    if (percent !== lastSent) {
      lastSent = percent;
      notify({ phase: 'download', percent, label: '下载中' });
    }
  }
  if (signal?.aborted) throw new DOMException('aborted', 'AbortError');
...
export async function executeTask(taskId, videoInfo, qualityIdx, deps) {
  ...
  const [audioBlob, videoBlob] = await Promise.all([
    downloadStream(bestAudio.baseUrl, signal, notify, fetchFn),
    downloadStream(bestVideo.baseUrl, signal, notify, fetchFn)
  ]);
...
Run: `Get-Content lib/download-core.js -Raw | node --input-type=module --check`
Expected: 无输出，exit code 0

- [ ] **Step 3: 手动冒烟（无 chrome 环境）**

Run: 单测式调用——用 node 以 ESM 导入验证导出符号：
```
...
```
Expected: `function function function bilibili_download`

- [ ] **Step 4: Commit**

```bash
git add lib/download-core.js
git commit -m "feat(download): 新增上下文无关的下载执行引擎 download-core"
```
