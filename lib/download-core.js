...
// B 站视频 CDN 防盗链要求 Referer 为 bilibili.com 域；
// offscreen 是 chrome-extension:// 页面，必须显式指定，否则流下载 403
...
export async function getPlayUrl(aid, bvid, cid, qn = 80, signal, fetchFn = fetch) {
  ...
  const r = await fetchFn(url, { credentials: 'include', referrer: BILI_REFERRER, signal });
